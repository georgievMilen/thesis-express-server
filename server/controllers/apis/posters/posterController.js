...
async function createPoster(req, res, next) {
  ...
  const uid = await modal(GET_UID_BY_EMAIL, email);
  if (uid.length === 0)
  ...
  const poster = await modal(POST_POSTER, post_data);
  if (poster.length < 1)
    next(ApiError.badRequest("A problem with the DB occured."));

  const poster_id = poster.insertId;
  const GET_REGIONS = GET_COUNTRY + Array(cities.length).join(" OR city = ?");
  const reg_ids = await modal(GET_REGIONS, [country, ...cities]);
  if (reg_ids.length === 0)
    next(ApiError.badRequest("A problem with the DB occured."));

  reg_ids.forEach(async ({ id: region_id }) => {
    await modal(INSERT_POSTER_REGION, [poster_id, region_id]);
  });
  if (genders.length > 0) {
    const GET_GENDERS =
      GET_GENDER_ID + Array(genders.length).join(" OR name = ?");
    const gen_ids = await modal(GET_GENDERS, [...genders]);
    if (gen_ids.length === 0)
      next(ApiError.badRequest("A problem with the DB occured."));
    gen_ids.forEach(async ({ id: gender_id }) => {
      await modal(INSERT_POSTER_GENDER, [poster_id, gender_id]);
    });
  }
...
function getAllPosters(req, res, next) {
  const posters = modal(GET_ALL_POSTERS_WUSER);
  const genders = modal(GET_ALL_PG);
  const regions = modal(GET_ALL_PR);
...
function getMyPosters(req, res, next) {
  ...
  const posters = modal(GET_MY_POSTERS, email);
  const genders = modal(GET_ALL_PG);
  const regions = modal(GET_ALL_PR);
  Promise.all([posters, genders, regions])
    .then((result) => {
...
async function sendConnectionRequest(req, res, next) {
  ...
  modal(CREATE_CONNECTION, [postId, user_id])
    .then(() => {
